...
function loadScaleModule(){
  ...
  const transformed = code.replace(/export const/g, 'const').replace(/export function/g, 'function') + '\nmodule.exports = { getKeySignature, scaleKeySignatures };';
  const mod = { exports: {} };
...
describe('getKeySignature', () => {
  const { getKeySignature } = loadScaleModule();
